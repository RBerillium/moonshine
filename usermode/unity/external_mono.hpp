...
#define OFFSET(func, type, offset) type func { return memory::read<type>( reinterpret_cast<uintptr_t>( this ) + offset ); } 
...
inline std::string read_widechar(const std::uintptr_t address, const std::size_t size) {
	...
	memory::read_memory(address, buffer.get(), size);
...
struct mono_table_info_t {
	int get_rows() {
		return memory::read<int>(reinterpret_cast<uintptr_t>(this) + 0x8) & 0xFFFFFF;
	}
};

struct mono_method_t {
	std::string name() {
		auto name = read_widechar(memory::read<uintptr_t>(reinterpret_cast<uintptr_t>(this) + 0x18), 128);
...
struct mono_class_field_t {
	OFFSET(offset(), int, 0x18)
		...
		std::string name() {
		auto name = read_widechar(memory::read<uintptr_t>(reinterpret_cast<uintptr_t>(this) + 0x8), 128);
		if (static_cast<std::uint8_t>(name[0]) == 0xEE) {
			char name_buff[32];
			sprintf_s(name_buff, 32, _("\\u%04X"), utf8_to_utf16(const_cast<char*>(name.c_str())));
			name = name_buff;
...
struct mono_vtable_t {
	OFFSET(flags(), byte, 0x30)
		...
		if ((this->flags() & 4) != 0)
			return memory::read<uintptr_t>(reinterpret_cast<uintptr_t>(this) + 0x40 + 8 * memory::read<int>(memory::read<uintptr_t>(reinterpret_cast<uintptr_t>(this) + 0x0) + 0x5c));
...
struct mono_class_t {
	OFFSET(num_fields(), int, 0x100)
		...
		std::string name() {
		auto name = read_widechar(memory::read<uintptr_t>(reinterpret_cast<uintptr_t>(this) + 0x48), 128);
		if (static_cast<std::uint8_t>(name[0]) == 0xEE) {
			char name_buff[32];
			sprintf_s(name_buff, 32, _("\\u%04X"), utf8_to_utf16(const_cast<char*>(name.c_str())));
			name = name_buff;
	...
	std::string namespace_name() {
		auto name = read_widechar(memory::read<uintptr_t>(reinterpret_cast<uintptr_t>(this) + 0x50), 128);
		if (static_cast<std::uint8_t>(name[0]) == 0xEE) {
			char name_buff[32];
			sprintf_s(name_buff, 32, _("\\u%04X"), utf8_to_utf16(const_cast<char*>(name.c_str())));
			name = name_buff;
	...
	int get_num_methods() {
		const auto v2 = (memory::read<int>(reinterpret_cast<uintptr_t>(this) + 0x2a) & 7) - 1;
		switch (v2) {
		case 0:
		case 1:
			return memory::read<int>(reinterpret_cast<uintptr_t>(this) + 0xFC);
		...
		case 4u:
			return memory::read<int>(reinterpret_cast<uintptr_t>(this) + 0xF0);
	...
	mono_method_t* get_method(const int i) {
		return reinterpret_cast<mono_method_t*>(memory::read<uintptr_t>(memory::read<uintptr_t>(reinterpret_cast<uintptr_t>(this) + 0xA0) + 0x8 * i));
	}

	mono_class_field_t* get_field(const int i) {
		return reinterpret_cast<mono_class_field_t*>(memory::read<uintptr_t>(reinterpret_cast<uintptr_t>(this) + 0x98) + 0x20 * i);
	}
...
namespace mono {
	inline mono_root_domain_t* get_root_domain() {
		return reinterpret_cast<mono_root_domain_t*>(memory::read<uintptr_t>(mono_copy + 0x499c78));
	}
	...
	inline mono_class_t* find_class(const char* assembly_name, const char* class_name) {
		...
		for (int i = 0; i < table_info->get_rows(); i++) {
			...
			if (!ptr->namespace_name().empty())
				name = ptr->namespace_name().append(_(".")).append(ptr->name());
